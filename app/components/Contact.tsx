        ...
        {links.map(({ label, value, href, external }) => (
          <li key={label}>
            <a
              ...
              className="group grid grid-cols-[6rem_1fr_auto] items-center gap-4 rounded-sm border border-border/40 bg-bg-elev px-4 py-3 neon-border hover:neon-border-hover sm:grid-cols-[8rem_1fr_auto] [@media(hover:none)]:neon-border-hover"
            >
              <span className="font-mono text-sm text-fg-muted">
                <span className="text-cyan">{">"}</span> {label}
              </span>
              <span className="truncate font-mono text-sm text-fg transition-colors group-hover:text-cyan [@media(hover:none)]:text-cyan">
                {value}
              </span>
              <svg
                aria-hidden="true"
                ...
                className="h-5 w-10 shrink-0 text-fg-muted/50 transition-colors duration-200 group-hover:animate-neon-blink-once group-hover:text-cyan group-focus-visible:animate-neon-blink-once group-focus-visible:text-cyan [@media(hover:none)]:text-cyan [@media(hover:none)]:[filter:drop-shadow(0_0_3px_rgba(0,240,255,0.85))_drop-shadow(0_0_10px_rgba(0,240,255,0.45))]"
              >
